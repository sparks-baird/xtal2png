...
# and `pip install xtal2png` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "xtal2png", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n xtal2png -f environment.docker.yml
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "xtal2png", "YOUR_CONSOLE_SCRIPT"]
